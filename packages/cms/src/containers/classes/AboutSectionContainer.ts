...
export const AboutSectionContainerPositions = [
	"top-right",
	"top-left",
	"bottom-right",
	"bottom-left"
...
export class AboutSectionContainer extends CmsJsonContainer<AboutSectionContainerContent> {
	...
	getDefaultImageFile() {
		const file = this.files.find(
			(f) => f.getIdentifier() === AboutSectionContainer.DEFAULT_IMAGE_IDENTIFIER
		);
		if (!file) return null;
		return file;
	}
	...
	getDesktopImageFile() {
		const file = this.files.find(
			(f) => f.getIdentifier() === AboutSectionContainer.DESKTOP_IMAGE_IDENTIFIER
		);
		if (!file) return null;
		return file;
	}
