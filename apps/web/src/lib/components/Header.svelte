	...
	const aboutItems = [
		{
			...
			description: "Try before you fly",
			icon: UsersSolid
		},
		{
			name: "ORGANISATION",
		...
		},
		{
			...
			description: "The joy of multiplayer action",
			icon: MapPinAltSolid
		},
		{
			name: "MERCH STORE",
			href: MERCH_URL,
			target: "_blank",
			description: "Show your true colours",
			icon: BagSolid
		}
	];
...
<Navbar fluid let:NavContainer class="bg-transparent text-gray-800 dark:bg-transparent">
	<div
		class="absolute left-0 top-0 z-0 h-full w-full bg-white/50 backdrop-blur-lg dark:bg-gray-900/75"
	></div>
	<NavContainer
		fluid
		class="z-30 sm:max-w-screen-sm md:max-w-4xl lg:max-w-screen-lg xl:max-w-screen-xl 2xl:max-w-screen-2xl"
	>
		<NavBrand href="/" class="ml-2">
			<img src={logo} class="me-3 h-7 drop-shadow sm:h-9" alt="Frontier Consolidated Logo" />
			...
			<div class="h-8 w-px sm:ml-8 sm:mr-4 lg:bg-gray-700"></div>
		</NavBrand>
		<div class="flex lg:order-2">
			...
			<a
				href={DISCORD_URL}
				class="dark:text-discord hover:text-discord dark:hover:text-discord-pressed ms-5 inline-flex shrink-0 items-center justify-center p-0 font-medium text-slate-800 xl:ms-10"
			>
		...
		<NavUl
			...
			class="order-1 mr-auto lg:ml-4 lg:mr-4 lg:flex-1 2xl:ml-8"
			ulClass="flex flex-col gap-1 lg:py-4 mt-4 lg:!flex-row lg:flex-wrap lg:!gap-x-8 lg:!gap-y-2 rtl:justify-end lg:mt-0 lg:text-sm lg:font-medium border-none bg-transparent dark:bg-transparent"
			{activeUrl}
			{activeClass}
			{nonActiveClass}
		>
			{#if !$user.data}
				<li class="sm:hidden">
					<button
						class={twMerge("block w-full rounded py-2 pe-4 ps-3 md:border-0 md:p-0", activeClass)}
						on:click={() => {
				...
				{#if !browser || matches}
					<NavLi
						class={twMerge(
							...
							activeUrl.startsWith("/about") ? activeClass : undefined
						)}
					>
						<InfoCircleSolid size="sm" tabindex="-1" />ABOUT US<ChevronDownOutline
							class={twMerge("h-3 w-3 transition-all", aboutOpen && "rotate-180")}
							tabindex="-1"
						/>
					</NavLi>
					{#if browser}
						<MegaMenu
							full
							offset={26}
							class="bg-gray-100/60 pb-4 backdrop-blur-xl dark:bg-slate-900/80"
						...
						>
							<a
								href={item.href}
								target={item.target}
								class="clip-r-4 block h-full border-l border-gray-700 p-4 transition hover:bg-gray-300 dark:border-gray-400 dark:hover:bg-gray-600"
							>
								<div class="text-md flex items-center font-medium text-black dark:text-gray-200">
									<svelte:component this={item.icon} size="sm" class="me-2" tabindex="-1" />
									{item.name}
									{#if item.target}
			...
			{#if !!$user.data}
				<NavLi href="/events" class="flex items-center gap-2">
					<CalendarMonthSolid size="sm" tabindex="-1" />EVENTS
				</NavLi>
			{/if}
			<NavLi href="/resources" class="flex items-center gap-2">
