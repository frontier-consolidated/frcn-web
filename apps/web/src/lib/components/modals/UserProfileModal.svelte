<script lang="ts">
	...
	$: joinDate = $userProfileView.data
		? new Intl.DateTimeFormat($locale!, {
				month: "long",
				year: "numeric"
			}).format(new Date($userProfileView.data.createdAt))
		: "Loading...";
	...
	$: allRoles =
		$userProfileView.open && $userProfileView.data ? getAllRoles($userProfileView.data) : [];
	$: canEdit = hasPermission($user.data?.permissions ?? 0, Permission.ManageRoles);

	let primaryRoleSearch = "";
	let nonPrimaryRoleSearch = "";
	$: giveableNonPrimaryRoles =
		$rolesCache.filter(
			(role) => !role.primary && !$userProfileView.data?.roles.find((r) => r.id === role.id)
		) ?? [];
	$: availablePrimaryRoles =
		$rolesCache.filter(
			(role) => role.primary && $userProfileView.data?.primaryRole.id !== role.id
		) ?? [];
</script>
...
>
	<div>
		<div class="mr-8 flex items-center gap-4">
			...
			<div class="flex flex-col">
				<span class="text-2xl font-semibold text-black dark:text-white"
					>{$userProfileView.data?.name ?? "Loading..."}</span
				>
		...
		<div class="flex flex-col gap-4">
			<a
				href={$userProfileView.data ? `discord://-/users/${$userProfileView.data.discordId}` : "/"}
				class="group flex items-center gap-2 rounded border border-gray-600 p-3 text-sm font-medium text-gray-700 dark:text-gray-400"
			>
				...
				<ArrowUpRightFromSquareOutline
					class="ml-auto group-hover:text-black dark:group-hover:text-gray-200"
					size="sm"
				/>
			</a>
			<div>
						...
						<Badge data-role-id={role.id}>
							...
							{#if canEdit}
								{#if primaryRole && availablePrimaryRoles.length > 0}
									<AngleDownSolid class="ms-2 h-2 w-2 cursor-pointer" />
