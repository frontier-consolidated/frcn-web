...
<Footer
	...
	class="relative flex flex-col items-center bg-zinc-200 dark:bg-gray-900 mt-12"
>
	<div class="absolute -top-[1.9rem] right-0 clip-tl-8 w-[40%] h-8 bg-zinc-200 dark:bg-gray-900"
	></div>
			...
			<div>
				...
				<FooterLinkGroup>
					{#each socials as social}
						<FooterLink liClass="mb-2" href={social.href} target="_blank"
							>{social.name}</FooterLink
						>
					{/each}
				</FooterLinkGroup>
			</div>
	...
	<div class="w-full max-w-6xl flex md:items-center gap-4 flex-col md:flex-row">
		<img
			...
			class="aspect-square w-16 mx-auto md:mx-0"
		/>
		<div class="max-w-xl">
			<FooterCopyright
				href="/"
			...
			<span class="block text-xs text-gray-500 dark:text-gray-400"
				>This is an unofficial Star Citizen site, not affiliated with the Cloud Imperium
		...
		<div class="relative flex space-x-6 rtl:space-x-reverse md:justify-center md:ml-auto pr-16">
			{#each socials as social}
				<FooterIcon href={social.href} target="_blank">
					<svelte:component
						this={social.icon}
						class="w-4 h-4 text-gray-500 dark:text-gray-500 hover:text-gray-900 dark:hover:text-white outline-none"
						tabindex="-1"
			...
			{/each}
			<button
				class="absolute rounded top-0 bottom-0 right-0 h-[180%] my-auto z-40 p-1 hover:bg-gray-400/30"
				on:click={() => {
