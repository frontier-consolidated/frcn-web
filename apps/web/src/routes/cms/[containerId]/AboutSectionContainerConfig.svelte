<script lang="ts">
	import {
		...
		CmsContainer,
		AboutSectionContainerPositions,
		CmsFile
	} from "@frcn/cms";
...
<Field
	{validator}
	for="about-section-identifier-{container.id}"
...
<Field {validator} for="about-section-content-{container.id}" value={editData.content}>
	...
	<MarkdownEditor placeholder="Content" bind:value={editData.content} />
</Field>
<div>
	<Label class="mb-2">CTAs</Label>
	<ContainerChildrenInput
		{validator}
		{container}
		addName="Add CTA"
		allowedChildren={[CMSContainerType.CallToAction]}
	/>
</div>
<div>
	<Label for="about-section-default-image-{container.id}" class="mb-2">Default Image</Label>
	<ImageInput
		id="about-section-default-image-{container.id}"
		name="about-section-default-image"
		src={defaultImageFile?.getSrc()}
		upload={async (file) => {
			defaultImageFile = await uploadFile(
				file,
				AboutSectionContainer.DEFAULT_IMAGE_IDENTIFIER,
				defaultImageFile
			);
		}}
		remove={async () => {
	...
	/>
	<Helper>Recommended image ratio is 2:1. Images will be scaled down to 1600w.</Helper>
</div>
<div>
	...
	<ImageInput
		id="about-section-default-image-{container.id}"
		name="about-section-default-image"
		src={desktopImageFile?.getSrc()}
		upload={async (file) => {
			desktopImageFile = await uploadFile(
				file,
				AboutSectionContainer.DESKTOP_IMAGE_IDENTIFIER,
				desktopImageFile
			);
		}}
		remove={async () => {
	...
	/>
	<Helper>
		This image will replace the default image on desktop. Leave as the placeholder image to use the
		default image on desktop.
	</Helper>
</div>
