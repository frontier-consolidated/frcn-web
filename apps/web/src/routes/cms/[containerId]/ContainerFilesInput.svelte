...
<section class="flex flex-col gap-2 rounded border border-gray-200 p-2 dark:border-gray-800">
	<div class="flex justify-end gap-2">
		<Button size="sm" on:click={() => (openUploadModal = true)}>Upload File</Button>
	</div>
	{#if files.length > 0}
		<Accordion>
			{#each files as file}
				...
				>
					<div slot="header" class="flex flex-1 items-center gap-4 pr-4">
						{#if file.contentType.startsWith("image/")}
							<img
								src={file.getSrc()}
								alt="Container file preview"
								class="h-full max-h-[4rem] rounded"
							/>
						{/if}
						<div class="flex flex-col">
							<span class="font-medium">{file.name}</span>
							<span class="text-xs text-gray-500">{file.id}</span>
						</div>
						<span class="ml-auto text-xs text-gray-500">{file.sizeKb}KB</span>
						<TrashBinSolid
	...
	{:else}
		<span class="block w-full py-4 text-center text-sm text-gray-500">No Files</span>
	{/if}
</section>
...
>
	<span class="block text-xs text-gray-500">File Id: {deleteFile?.id}</span>
	<span
		>Are you sure you want to delete the <strong>{deleteFile?.name}</strong> file? Once deleted it cannot
		be undone.</span
	>
</ConfirmationModal>
