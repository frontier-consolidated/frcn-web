...
<div class="flex flex-col gap-4">
	...
	<div class="flex flex-wrap gap-4">
		...
		<span class="text-sm font-semibold dark:text-gray-400">
			{#if data.duration}
				{#key data.duration}
					Duration: <TimeBadge id="event-duration-badge" format="duration" value={data.duration} />
				{/key}
			{/if}
		</span>
	</div>
	{#if data.imageUrl}
		<img src={data.imageUrl} alt="{data.name} thumbnail" class="max-w-[40rem] rounded-lg" />
	{/if}
	<div class="max-w-[60rem]">
		<h2 class="mt-4 text-lg font-semibold dark:text-white">Details</h2>
		<ViewMore>
			<Markdown
