<script lang="ts">
	...
	$: {
		isDirty =
			editData.description !== data.key.description ||
			editData.permissions !== data.key.permissions;
		canNavigate.set(!isDirty);
	}

	let key = $page.state.newAccessKey?.key ?? null;
	if ($page.state.newAccessKey?.key)
		replaceState("", {
	...
	<div class="flex flex-col gap-4 p-4">
		...
		<div>
			...
			<div class="mt-1 flex gap-2">
				{#if key}
					<Button size="sm" on:click={copy}>Copy</Button>
				{/if}
				<Button
					size="sm"
					on:click={() => {
		...
		<div>
			...
			<PermissionToggles
				disableToggles={{ [Permission.Admin]: !hasAdmin($user.data?.permissions ?? 0) }}
