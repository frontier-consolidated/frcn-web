...
<div class="flex flex-1 flex-col justify-between" use:initNavigation>
	<div class="flex flex-col gap-4 p-4">
		<Field {validator} for="system-general-guildid" value="a" required>
			...
			>
				<button
					...
					on:click={() => (guildIdFieldLocked = !guildIdFieldLocked)}
				>
					<svelte:component
						this={guildIdFieldLocked ? LockSolid : LockOpenSolid}
						tabindex="-1"
						class="outline-none"
						size="sm"
					/>
		...
		<div>
			...
			<Button href="/admin/general/accesskeys" color="dark" size="sm">Manage Access Keys</Button>
		</div>
	</div>
