...
>
	...
	<TableBodyCell class="text-center group-[.dragging]:hidden dark:text-gray-400">
		<div class="flex flex-wrap items-center justify-center gap-2 text-xs">
			<Tooltip>
				<div slot="icon" class="flex items-center">
					<span>{channel.events.filter((e) => e.state === EventState.Started).length}</span>
					<Indicator color="green" size="xs" class="relative ms-1">
						<Indicator color="green" size="xs" class="absolute left-0 top-0 animate-ping" />
					</Indicator>
				</div>
...
<ConfirmationModal
	title="Delete Channel Link - {channel.discord.name}"
	bind:open={modalOpen}
	on:confirm={async () => {
...
>
	<p>
		Are you sure you want to delete the link to the <strong>{channel.discord.name}</strong> event channel?
		Once deleted it cannot be undone.
	</p>
	<p>
		Any scheduled events posted in this channel will have to be reposted in another event channel.
	</p>
