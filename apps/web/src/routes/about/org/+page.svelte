...
<svelte:head>
	<title>{index?.getMetaTitle() ?? "Our Organisation - About"} | Frontier Consolidated</title>
...
<PageHero srcset={heroImageSrcset}>
	<Heading tag="h1" class="text-white font-medium text-4xl sm:text-5xl drop-shadow-md">{index?.getTitle() ?? "Frontier Organisation"}</Heading>
