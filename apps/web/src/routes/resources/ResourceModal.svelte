<script lang="ts">
	...
	$: isDirty =
		...
		(data.name !== editData.name ||
			data.shortDescription !== editData.shortDescription ||
			data.tags.length !== editData.tags.length ||
			data.tags.some((tag) => !editData.tags.find((t) => t === tag)));
...
<Modal title={data ? `Edit - ${data.name}` : "Upload resource"} bind:open dismissable>
	<div class="flex flex-col gap-4 p-4">
		<Field {validator} for="resource-upload-name" value={editData.name} required>
		...
		<Field {validator} for="resource-upload-description" value={editData.shortDescription}>
			<Label for="resource-upload-description" class="mb-2">Description</Label>
			<Textarea
				class="rounded border-gray-300 bg-gray-50 text-gray-900 dark:border-gray-500 dark:bg-gray-600 dark:text-white dark:placeholder-gray-400"
		...
		<Field {validator} for="resource-upload-tags" value={editData.tags} required>
		...
		{#if !data}
			<Field {validator} for="resource-upload" value={uploadFiles} required>
				...
				<button
					type="button"
					class="dark:hover:bg-bray-800 relative flex h-64 w-full items-center justify-center rounded border-2 border-dashed border-gray-300 bg-gray-50 hover:bg-gray-100 dark:border-gray-600 dark:bg-gray-700 dark:hover:border-gray-500 dark:hover:bg-gray-600"
						...
						<div class="flex flex-col items-center">
							{#if uploadFiles[0].type.startsWith("image")}
								<img
									...
									alt="Preview image of '{uploadFiles[0].name}' file"
									class="h-32 rounded"
								/>
							{:else}
