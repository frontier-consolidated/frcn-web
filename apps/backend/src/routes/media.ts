	...
	try {
		...
		if (e.code === "EEXIST") {
			if (!fs.statSync(cache.basePath).isDirectory()) {
				throw new Error(
					`Cannot create directory ${cache.basePath}, a file already exists at this position`
				);
			}
			return;
		}
				...
				switch (type.toLowerCase()) {
					case "resource": {
						...
						if (!resource) {
							return res
								.status(404)
								.send({ message: `Could not find resource 'attach_to=${attach_to}'` });
						}

						await $files.uploadFile(
							context.s3Client,
							config.files.bucketName,
							file,
							req.user!,
							async (tx, fileUpload) => {
					...
					case "cms_container": {
						...
						if (!container) {
							return res
								.status(404)
								.send({ message: `Could not find cms container 'attach_to=${attach_to}'` });
						}
						...
						const uploadedFile = await $cms.attachFile(
							context.s3Client,
							config.files.bucketName,
							file,
							req.user!,
