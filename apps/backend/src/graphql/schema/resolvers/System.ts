...
export const systemResolvers: Resolvers = {
	...
	Mutation: {
		async editSystemSettings(source, args, context) {
			...
			if (
				args.data.defaultEventChannelId &&
				!(await $events.getEventChannel(args.data.defaultEventChannelId))
			) {
		...
		async createEventChannel(source, args, context) {
			...
			if (args.existingReadyRoomId) {
				existingReadyRoom = await $discord.getChannel(
					context.app.discordClient,
					args.existingReadyRoomId,
					args.guildId
				);
			...
			const channel = await $events.createEventChannel(
				discordGuild,
				discordChannel,
				discordCategory,
				existingReadyRoom ?? undefined
			);
			return resolveEventChannel(channel);
		},
			...
			if (args.data.categoryId) {
				...
				if (discordCategory.type !== ChannelType.GuildCategory) {
					throw gqlErrorBadInput(
						"Given id for event category does not point to a category channel"
					);
				}
			}
		...
		async deleteEventChannel(source, args, context) {
			...
			await $events.deleteEventChannel(
				channel,
				context.app.discordClient,
				args.deleteVoiceChannels ?? false
			);
			return true;
		},
		...
		async editAccessKey(source, args, context) {
			...
			if (
				...
				!hasAdmin(await calculatePermissions(context)) &&
				hasAdmin(args.data.permissions)
			) {
