...
const attachmentConfigs = {
	...
	string,
	{
		permission: Permission | { one: Permission[] } | { all: Permission[] };
		allowedFiles?: string[];
	}
>;
...
export function fileField(
	...
	const handler =
		options?.disk === false
			? memUpload.array(field, options?.maxFiles)
			: diskUpload.array(field, options?.maxFiles);
	...
	return async function (req, res, next) {
		...
		if (options?.requireAttachment) {
			...
			if (typeof config.permission === "object") {
				if (
					"one" in config.permission &&
					!hasOneOfPermissions(permissions, config.permission.one)
				) {
					canUpload = false;
				} else if (
					"all" in config.permission &&
					!hasAllOfPermissions(permissions, config.permission.all)
				) {
					canUpload = false;
				}
				...
				for (const file of files) {
					...
					if (
						!(
							...
							allowedFiles.includes(`${baseMimeType}/*`)
						)
					) {
